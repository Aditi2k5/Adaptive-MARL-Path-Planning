...
def build_next_rider_order_map(env: QuickCommerceEnv) -> Dict[int, Order]:
    """
    FIX: builds the REAL post-step rider→order pairing, mirroring the
    exact same urgency-sorted assignment logic used for the current-step
    obs_list. Previously this was always {} (empty dict), forcing
    next_obs to be all-zero regardless of the actual next state.

    Only riders who are idle AND have a pending order to be assigned
    get a real next-observation; riders still mid-delivery correctly
    get zero-padding since they have no next action to take yet.
    """
    next_idle    = env.idle_riders()
    next_pending = env.pending_orders()
    if not next_idle or not next_pending:
        return {}
    next_pending_sorted = sorted(
        next_pending, key=lambda o: o.urgency(env.t), reverse=True
    )
...
def train(city_name: str = "bangalore",
    ...
    for ep in tqdm(range(start_ep, n_episodes + 1), desc="Training",
        ...
        while env.t < config.EPISODE_LENGTH:
            ...
            # ── FIX: build genuine next-observations from real post-step state ──
            next_rider_order_map = build_next_rider_order_map(env)
            next_obs = collect_observations(env, next_rider_order_map, network)

            obs_arr    = np.stack(obs_list)
            acts_arr   = np.array(actions)
            next_arr   = np.stack(next_obs)
